...
namespace Interneuron.CareRecord.HL7SynapseHandler.Service.AutoMapperProfiles
{
    public class OrderMapperProfile : Profile
    {
        ...
        private FhirDateTime GetEffective(entitystorematerialised_CoreOrder coreOrder)
        {
            ...
            {
                var elem = new FhirDateTime(coreOrder.Datetimeoftransaction.Value.Year, coreOrder.Datetimeoftransaction.Value.Month, coreOrder.Datetimeoftransaction.Value.Day, coreOrder.Datetimeoftransaction.Value.Hour, coreOrder.Datetimeoftransaction.Value.Minute, coreOrder.Datetimeoftransaction.Value.Second, TimeSpan.Zero);
    ...
    internal class ReportContainedResolver : IValueResolver<entitystorematerialised_CoreResult, DiagnosticReport, List<Resource>>
    {
        ...
        private List<Observation.ReferenceRangeComponent> GetReferenceRange(entitystorematerialised_CoreResult coreResult)
        {
            ...
            if (coreResult.Referencerangelow.IsNotEmpty())
            {
                ...
                {
                    Value = Decimal.TryParse(coreResult.Referencerangelow, out Decimal refLow) ? Math.Round(refLow, 2) : default(decimal?)
                };
            }
            ...
            if (coreResult.Referencerangehigh.IsNotEmpty())
            {
                ...
                {
                    Value = Decimal.TryParse(coreResult.Referencerangehigh, out Decimal refHigh) ? Math.Round(refHigh, 2) : default(decimal?)
                };
            }
        ...
        private DataType GetEffective(entitystorematerialised_CoreResult coreResult)
        {
            ...
            {
                var elem = new FhirDateTime(coreResult.Observationdatetime.Value.Year, coreResult.Observationdatetime.Value.Month, coreResult.Observationdatetime.Value.Day, coreResult.Observationdatetime.Value.Hour, coreResult.Observationdatetime.Value.Minute, coreResult.Observationdatetime.Value.Second, TimeSpan.Zero);
                elem.ElementId = "effectiveDateTime";
                return elem;
            }
            return null;
