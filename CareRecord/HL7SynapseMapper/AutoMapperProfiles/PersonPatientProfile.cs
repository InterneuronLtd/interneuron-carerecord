...
namespace Interneuron.CareRecord.HL7SynapseService.AutoMapMapperProfiles
{
    public class PersonPatientProfile : Profile
    {
        public PersonPatientProfile()
        {
            CreateMap<entitystorematerialised_CorePerson, Patient>()
                ...
                .ForMember(dest => dest.BirthDate, opt => 
                {
                    ...
                    opt.MapFrom(src => ((DateTime)src.Dateofbirth).ToString("yyyy-MM-dd"));
                })
                ...
                {
                    new HumanName().WithGiven(src.Firstname).WithGiven(src.Middlename).AndFamily(src.Familyname).WithPrefix(src.Titletext)
                }))
                ...
                .ForMember(dest => dest.Extension, opt => opt.MapFrom(src => GetPersonExtensions(src)));
        ...
        private Element DeathIndicatorMapper(entitystorematerialised_CorePerson src, Patient patient)
        {
            ...
            { 
                element = new FhirDateTime(((DateTime)src.Dateofdeath).ToString("yyyy-MM-dd")); 
            }
        ...
        private List<Extension> GetPersonExtensions(entitystorematerialised_CorePerson src)
        {
            var extensions = new List<Extension>();

            if (src.Ethnicitycode.IsNotEmpty() && src.Ethnicitytext.IsNotEmpty())
            {
                var extension = new Extension();
                extension.Url = "Ethinicity";
                extension.Value = new FhirString(src.Ethnicitytext);
                extension.ElementId = src.Ethnicitycode;
